...
public class AlarmMultiPlayer implements MediaPlayer.OnCompletionListener {
    ...
    private void handleSetDataSourceUri(Uri uri) {
        ...
        if (Utils.isPathPrefixMatch(uri, Audio.Media.EXTERNAL_CONTENT_URI)
                || Utils.isPathPrefixMatch(uri, Audio.Media.INTERNAL_CONTENT_URI)) {
            columnName = Audio.Media._ID;
        } else if (Utils.isPathPrefixMatch(uri, Audio.Playlists.EXTERNAL_CONTENT_URI)
                || Utils.isPathPrefixMatch(uri, Audio.Playlists.INTERNAL_CONTENT_URI)) {
